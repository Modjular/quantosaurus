...
export class CellposeWebGPU {
  ...
  async segmentImage(gray, H, W, opts = {}) {
    ...
    if (Math.abs(rescale - 1) > 1e-6) {
      H2 = Math.max(1, Math.round(H * rescale));
      W2 = Math.max(1, Math.round(W * rescale));
      // clamp to what the GPU can actually allocate: the decoder's widest full-resolution
      // intermediate buffer is 64 channels, and several such buffers are live at once during
      // a forward pass, so a single-buffer byte-limit check isn't enough headroom — pushing
      // close to it crashes bindgroup creation under real GPU memory pressure (observed with
      // Chromium/ANGLE-Metal), not just the buffer allocation itself. A too-small diameter
      // must not be allowed to blow past what's actually safe, since exceeding limits either
      ...
      if (H2 * W2 > maxPixels) {
        ...
        const reason = byDeviceLimit < practicalCap ? "the GPU's buffer limit"
          : `the ${practicalCap.toLocaleString()}-pixel working-resolution cap`;
        console.warn(`segmentImage: diameter=${diameter} requests a ${H2}x${W2} working resolution, ` +
          `exceeding ${reason}; clamped to ${clampedH2}x${clampedW2}.`);
    ...
    for (let y = 0; y < H2; y++) for (let x = 0; x < W2; x++) {
      dP2[y * W2 + x] = output[y * Wp + x];                       // ch0
      dP2[H2 * W2 + y * W2 + x] = output[Hp * Wp + y * Wp + x];   // ch1
      cellprob2[y * W2 + x] = output[2 * Hp * Wp + y * Wp + x];   // ch2
    }
